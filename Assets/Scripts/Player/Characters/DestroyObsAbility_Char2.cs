...
public class DestroyObsAbility_Char2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Obstacle")
        {
            ...
            if (obs != null)
            {
                ...
                if (obs.GetObstacleType() == ObstacleType.Upper)
                {
                    StartCoroutine(DelayDestroyObsAfterSecs(obs, 0.05f)); // Delay destroy for 0.05 seconds
